...
!$OMP END PARALLEL DO
    ...
    !huoxiao adjoint emission output
    IF ( LADJ_EMS ) THEN
       ALLOCATE( ADJ_FLUX(NI, NJ,  1 ), STAT = ADJRC )
       IF ( BaseDct%Dta%ncFile(20:25) == 'FOSSIL') THEN
          !original fossil flux
          VarName = 'co2_original_fossil_flux'
          ADJ_FLUX(:, :, 1) = OUTARR_3D(:, :, 1)
          CALL WRITE_FILE(VarName, ADJ_FLUX)
          IF ( LADJ_FOSSIL .OR. LADJ_FEMISS ) THEN
          OUTARR_3D(:, :, 1) = EMS_SF(:, :, 1)*OUTARR_3D(:, :, 1)      
          !scaled fossil flux
          VarName = 'co2_scaled_fossil_flux'
          ADJ_FLUX(:, :, 1) = OUTARR_3D(:, :, 1)
          CALL WRITE_FILE(VarName, ADJ_FLUX)
          ENDIF !fossil
       ELSEIF ( BaseDct%Dta%ncFile(20:24) == 'OCEAN') THEN          
          !original ocean flux
          VarName = 'co2_original_ocean_flux'
          ADJ_FLUX(:, :, 1) = OUTARR_3D(:, :, 1)
          CALL WRITE_FILE(VarName, ADJ_FLUX)
          ...
          CALL WRITE_FILE(VarName, ADJ_FLUX)
          ENDIF !ocean
       ELSEIF ( BaseDct%Dta%ncFile(20:22) == 'BIO' .AND. BaseDct%Dta%ncFile(20:25) /= 'BIOFUE') THEN
          !original bio flux
          VarName = 'co2_original_bio_flux'
          ADJ_FLUX(:, :, 1) = OUTARR_3D(:, :, 1)
          CALL WRITE_FILE(VarName, ADJ_FLUX)
          ...
          CALL WRITE_FILE(VarName, ADJ_FLUX)
          ENDIF !bio
       ELSEIF ( BaseDct%Dta%ncFile(20:25) == 'BIOFUE') THEN
          !original biofuel flux
          VarName = 'co2_original_biofuel_flux'
          ADJ_FLUX(:, :, 1) = OUTARR_3D(:, :, 1)
          CALL WRITE_FILE(VarName, ADJ_FLUX)
          ...
          ENDIF !biofuel
       ENDIF
       IF ( ASSOCIATED( ADJ_FLUX ) ) DEALLOCATE( ADJ_FLUX)
       ADJ_FLUX => NULL()
    ENDIF
!    FILE_PREFIX = 'HUOXIAO'
!    CALL WRITE_FILE(FILE_PREFIX, OUTARR_3D, nI, nJ, nL )
