...
!assume  
      ...
      SUBROUTINE READ_TANSAT_CO2_OBS( FILENAME, NTANSAT ) 
!
!******************************************************************************
...
!      CALL Ncrd(pre_varrd_2dr, FID, TRIM(varname), pre_str2d, pre_cnt2d)
     ...
     &                        err_stop, stat)
      ...
      DO N = 1, NTANSAT
         ...
         TANSAT(N)%S_OER_INV        = 1/Sinv_varrd_1dr(N)
...
!        END DO
      ...
      END SUBROUTINE READ_TANSAT_CO2_OBS
...
!      USE PRESSURE_MOD,       ONLY : GET_PCENTER, GET_PEDGE
      ...
      TANSAT_FILENAME = "../TANSAT_CO2/tansat_"//TRIM(ADJUSTL(CHA_NYMD)) &
...
!      OPEN(UNIT=1997, FILE='ADJOINT', ACCESS='APPEND')
      ...
      OPEN(UNIT=1995, FILE='OBS_ORI_INC_COST_FUNCTION', &
           POSITION='APPEND')
!      OPEN(UNIT=1996, FILE='OBS_ORI_ADJOINT', POSITION='APPEND')
!    print*, 'huoxiao_debug NTANSAT', NTANSAT
      DO NT  = 1, NTANSAT
...
!                                         LTANSAT)
         ...
         NEW_COST(NT) = 0.d0
         NEW_COST(NT) = NEW_COST(NT)+0.5*FORCE*(GC_XCO2-TANSAT(NT)%XCO2)
...
!            ENDDO
        ...
        ! Pass adjoint back to adjoint tracer array
         State_Chm_Adj%SpeciesAdj(I,J,:,IDTCO2)  = State_Chm_Adj%SpeciesAdj(I,J,:,IDTCO2) &        
                                                 + GC_CO2_NATIVE_ADJ(:)
!         WRITE(1996, *)State_Chm_Adj%SpeciesAdj(I,J,1,IDTCO2)
         ! huoxiao debug
        ! finite difference
!         IF ( FINITE_DIFF ) THEN 
!            print*, 'finite diff' 
