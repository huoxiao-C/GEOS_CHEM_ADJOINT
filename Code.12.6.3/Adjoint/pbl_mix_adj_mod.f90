...
!$OMP PARALLEL DO &
!$OMP DEFAULT( PRIVATE ) &
!$OMP SHARED( tcd, A, State_Chm_Adj, State_Met_Adj, State_Grid_Adj ) &
!$OMP SHARED( dtcd, NX, NY, NTRC )
      DO J = 1, NY
      DO I = 1, NX
      !tangent validation 
...
!$OMP PARALLEL DO &
!$OMP DEFAULT( PRIVATE ) &
!$OMP SHARED( State_Chm_Adj, State_Met_Adj, State_Grid_Adj ) &
!$OMP SHARED( A, adtc, adtc_in, adtc_out ) &
!$OMP SHARED( NX, NY, NTRC)
      ! Loop over Lat/Long grid boxes (I,J)
      DO J = 1, NY
      DO I = 1, NX
